...
class DNSResolver:
    ...
    def _filter_servers_by_ip_version(self, version: int) -> List[Tuple[str, int]]:
        servers = []
        for server in self.dns_servers:
            try:
                ip_str = server[0]
    ...
    def get_ipv6_servers(self) -> List[Tuple[str, int]]:
        return self._filter_servers_by_ip_version(6)

    def get_ipv4_servers(self) -> List[Tuple[str, int]]:
        return self._filter_servers_by_ip_version(4)
    ...
    def _rate_limit(self):
        with self._rate_limit_lock:
            ...
            if current_time - self._last_query_time >= 1:
                self._query_count = 0
                self._last_query_time = current_time
            
            self._query_count += 1
            if self._query_count > self.rate_limit_per_second:
                sleep_time = 1 - (current_time - self._last_query_time)
                if sleep_time > 0:
    ...
    def _parse_name(
        ...
        while True:
            ...
            label_count += 1
            if label_count > self.MAX_TOTAL_LABELS:
                raise ValueError("Too many labels in DNS name")
    ...
    def _parse_response(self, data: bytes, tid: int) -> List[DNSRecord]:
        ...
        if len(data) > self.MAX_RESPONSE_SIZE:
            raise ValueError(f"DNS response too large: {len(data)} bytes")
    ...
    def _send_query(self, query: bytes, server: Tuple[str, int]) -> bytes:
        if self.use_tcp:
            ...
            try:
                ...
                if response_length > self.MAX_RESPONSE_SIZE:
                    raise ValueError(f"Response length {response_length} exceeds maximum")
            ...
            for family, socktype, proto, canonname, sockaddr in addr_info:
                try:
                    with socket.socket(family, socktype, proto) as s:
                        ...
                        if len(data) > self.MAX_RESPONSE_SIZE:
                            raise ValueError(f"Response size {len(data)} exceeds maximum")
    ...
    def _get_from_cache(self, cache_key: str) -> Optional[List[DNSRecord]]:
        ...
        with self._cache_lock:
            ...
            if entry:
                if time.time() - entry.timestamp < entry.min_ttl:
    ...
    def _add_to_cache(self, cache_key: str, records: List[DNSRecord]):
        ...
        min_ttl = min((r.ttl for r in records if r.ttl > 0), default=0)
        if min_ttl == 0:
            return
